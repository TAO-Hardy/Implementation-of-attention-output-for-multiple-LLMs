...
dir_name = 'llama_qa'
model_dir = "/data/LLM/llama-7b-hf"

tokenizer = LlamaTokenizer.from_pretrained(model_dir)
model = LlamaForCausalLM.from_pretrained(model_dir, output_attentions=True)
...
if __name__ == '__main__':
    ...
    for question in questions:
        for context in contexts:
            ...
            attention_cross = attentions[-s:, s:e]#the attention score for context from every tokens which included in quesions.
...
#output the attention score for context from every tokens which included in quesions. file is named as cross_i.txt 
            ...
            print(attention_context.shape, attention_qa_to_context.shape)

            input_ids = tokenizer.encode('%s Question: %s Answer:' % (context, question), return_tensors="pt")
